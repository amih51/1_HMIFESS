              ...
              {comment.isAnon ? (
                <>
                  ...
                  <div className="flex flex-col">
                    ...
                    {/*Updated At*/}
                    <p className="ml-1 text-[10px] text-gray-400 font-normal">{new Date(comment.createdAt).toLocaleString()} <br /></p>
                  </div>
                </>
              ) : (
                <>
                  <Link href={`/profile/${comment.user.id}`}>
                    <Image src={comment.user.image} alt="profile photo" width={40} height={40} style={imageStyle} />
                  </Link>
                  <div className="flex flex-col">
                    <Link href={`/profile/${comment.user.id}`} className="ml-3 hover:underline">
                      {comment.user.name}
                    </Link>
                    <p className="ml-3 text-[10px] text-gray-400 font-normal">{new Date(comment.createdAt).toLocaleString()} <br /></p>
                  </div>
                </>
              )}
            </div>
